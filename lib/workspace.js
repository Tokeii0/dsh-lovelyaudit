...
export const PHASES = [
  { id: 'P0', name: '交战规则', goal: '钉死授权范围、实例台账与非破坏红线', skill: 'audit-methodology' },
  { id: 'P1', name: '指纹', goal: '识别产品/版本，拉取已知 CVE 候选池', skill: 'blackbox-testing' },
  { id: 'P2', name: '测绘', goal: '路由/JSP/组件/残留/Source-Sink 五张清单', skill: 'blackbox-testing' },
  { id: 'P3', name: '黑盒', goal: '可达面地图与鉴权边界行为表', skill: 'blackbox-testing' },
  { id: 'P4', name: '代审', goal: '污点连线、配置根因、依赖 CVE', skill: 'code-audit' },
  { id: 'P5', name: '互证', goal: '可达性叠加缺陷，诚实定级', skill: 'audit-methodology' },
  { id: 'P6', name: '验证', goal: '最小非破坏 PoC，尝试放大影响', skill: 'blackbox-testing' },
  { id: 'P7', name: '闭环', goal: '入口×类型矩阵无空格后收工', skill: 'vuln-coverage' },
]
...
export const COVER_TYPES = ['RCE', 'SQLi', '文件', 'SSRF', 'XXE', 'XSS', '越权', '重定向', '密码学', '逻辑']
...
function applyFindingExtras(item, args) {
  ...
  if (args.severity !== undefined) {
    const severity = textOf(args.severity, item.severity || 'Info')
...
function buildFinding(args, id, workspace) {
  ...
  const item = {
    ...
    grade,
    severity: SEVERITIES.includes(textOf(args.severity, '')) ? args.severity : 'Info',
    verifyStatus: 'draft',
