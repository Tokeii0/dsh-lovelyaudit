...
/** Windows device names stay unusable as file or directory names even with an extension. */
...
export function safeFilePart(value, fallback = 'untitled') {
  ...
  // A part made only of dots ("." / "..") resolves to the current or PARENT directory,
  // which would write reports outside the intended root.
  if (text === '' || /^\.+$/.test(text)) return fallback
  if (RESERVED_NAMES.test(text)) return `_${text}`
  return text
}
