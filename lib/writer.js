...
export function reportWriterPrompt(workspace, finding, reason) {
  ...
  return [
    ...
    '报告逻辑必须对齐标准《源码安全审计报告》的单条漏洞写法（CWE + OWASP 编号、CVSS 向量）：标题、风险等级/CWE/OWASP、位置、代码片段、判断依据（三点闭合编号）、影响、修复。',
    ...
    '修复一节必须分三层：临时缓解（今天能上，写明这只是缓解）、根治（给出正确代码写法，不要写「请加强过滤」这类废话）、变体清单（同一模式在项目中的其它位置；台账 variants 为空就写「未做变体分析」，不要编）。台账有 rootCause 时单列根因段。',
    '风险等级行同时给 severity、CWE、OWASP，台账有 cvss 就把向量原样带上。P5 语义级（unauth/session/key/blocked/code）必须出现，且不得把白盒结论写成未授权可打。',
