...
/**
 * Per-turn view of the 目标 board. The subgoal instruction used to live only in the kickoff
 * message, so on a long run it fell out of context and the board stopped being updated.
 * Subgoal titles already carry their code (numberedSubgoalTitle), so do not print it twice.
 */
export function campaignText(workspace) {
  const campaign = workspace && workspace.campaign ? workspace.campaign : null
  if (!campaign) return ''
  const subgoals = Array.isArray(campaign.subgoals) ? campaign.subgoals : []
  const lines = ['目标纪律：每轮都要更新「目标」页——新拆的用 add_subgoal，正在做的 update_subgoal 标 active，做完立刻 status=done 销号。编号走 P2.1 / P2.2.3 层级，可用 parentId 挂在上一条下面。']
  if (!campaign.goal && subgoals.length === 0) {
    lines.push('目标页现在是空的：先 set_campaign 写总目标与当前焦点，再按当前阶段 add_subgoal 拆 2–4 条。')
    return lines.join('\n')
  }
  if (campaign.goal) lines.push(`总目标：${campaign.goal}`)
  if (campaign.current) lines.push(`当前焦点：${campaign.current}`)
  const open = subgoals.filter((item) => item && item.status !== 'done')
  const active = open.filter((item) => item.status === 'active').slice(0, 3)
  const pending = open.filter((item) => item.status !== 'active').slice(0, 5)
  if (active.length) {
    lines.push('进行中：')
    for (const item of active) lines.push(`  [~] ${item.title}`)
  }
  if (pending.length) {
    lines.push('待办：')
    for (const item of pending) lines.push(`  [ ] ${item.title}`)
  }
  lines.push(`进度：${subgoals.length - open.length}/${subgoals.length} 已销号。`)
  if (subgoals.length && open.length === 0) {
    lines.push('本阶段子目标已清空——要么 add_subgoal 补下一批，要么 set_phase 进入下一阶段。')
  }
  return lines.join('\n')
}
...
export function remindIdeas(workspace) {
  return [
    ...
    '漏洞纪律：先 add_finding 且 verifyStatus=draft。必须同时有 rationale、reachable、evidence 才能 verified。闭合时补 poc/exp，报告要能直接复现。severity 必须自己给（Critical/High/Medium/Low），别漏；grade 是可达性、severity 是影响面，两个轴都要填。',
  ].join('\n')
}
