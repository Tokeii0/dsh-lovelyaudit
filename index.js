...
const PROMPT_TEXT = [
  ...
  'Reporting a verified finding immediately spawns a writer subagent that produces a dedicated 源码安全审计报告 for THAT finding only (standard per-vulnerability SRC-nn layout: title, CWE/OWASP, location, snippet, three-point rationale, impact, fix, and a copy-paste PoC/EXP). Do not wait until P7. Each vuln has its own report; never merge them. Writer subagents can report back to you.',
  ...
  'grade and severity are two DIFFERENT axes and you must set BOTH on every add_finding. grade = reachability (unauth/session/key/blocked/code). severity = impact (Critical/High/Medium/Low/Info). Omitting severity is not neutral — it derives a floor from grade that is usually too low. A default-credential admin takeover or an unauthenticated dump of every project config is Critical, not Info. Info means "not a vulnerability, hardening advice only" and is rejected outright with grade unauth or session. Also fill cwe and owasp; load audit-reporting for the mapping table.',
  UNKNOWN_VULN_SYSTEM,
  ...
  ctx.tools.register({
    ...
    parameters: {
      ...
      properties: {
        ...
        grade: {
          type: 'string',
          enum: ['unauth', 'session', 'key', 'blocked', 'code'],
          description: 'P5 corroboration grade. unauth = reachable with no credentials. session = any logged-in account (say whether registration is open). key = only a secret/token is missing. blocked = defect is real but the current boundary or WAF stops it. code = white-box defect not reproduced live. This is reachability, NOT impact — set severity separately.',
        },
        source: { type: 'string' },
        sink: { type: 'string' },
        ...
        severity: {
          ...
          description: 'Impact level. REQUIRED on add_finding — omitting it derives a floor from grade, which is rarely the right level. Critical = unauthenticated RCE / full data exposure / account takeover / cross-tenant read. High = unauthenticated read of sensitive data, authenticated privilege escalation, stored XSS on an authenticated page. Medium = needs a session or a key, or limited blast radius. Low = minor, or the defect is currently blocked. Info = NOT a vulnerability (hardening advice only) — never use it with grade unauth or session.',
        },
        verifyStatus: {
          type: 'string',
          enum: ['draft', 'unverified', 'verified', 'blocked'],
          description: 'verified requires all three of rationale, reachable and evidence; otherwise it is forced back to draft.',
        },
        cwe: { type: 'string' },
        owasp: { type: 'string' },
        cvss: { type: 'string' },
